...
void main() async {
  print('=== Diagrammatic Visualization Service Test ===');

  // Test with the API key from constants
  final apiKey = AppConstants.groqApiKey;
  print('Using API key: ${apiKey.substring(0, 10)}...');

  if (apiKey == 'notepad lms_app/lib/test_diagrammatic_visualization.dart' || apiKey.isEmpty) {
    print(
  ...
  try {
    ...
    final flowchart = await diagramService.generateDiagramDescription(
      code:
    ...
    print('\n--- Test 2: Styled Explanation with Diagram ---');
    final styledExplanation = await diagramService.generateStyledExplanation(
      code:
          'def fibonacci(n):\n    if n <= 1:\n        return n\n    else:\n        return fibonacci(n-1) + fibonacci(n-2)\n\nresult = fibonacci(5)\nprint(result)',
      language: 'Python',
      diagramDescription: flowchart,
      diagramType: DiagramType.flowchart,
    );
    print('Styled Explanation:');
    print(styledExplanation);

    print('\n--- Test 3: Full Execution Visualization ---');
    ...
    print(
      '\n=== All diagrammatic visualization tests completed successfully ===',
    );
    ...
    print('✅ State diagram generation');
    print('✅ Styled explanations with emojis');
